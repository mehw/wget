            ...
            for (murl_ptr = mfile->metaurls; *murl_ptr; murl_ptr++)
              {
                ...
                if (_metaurl > 0)
                  {
                    if (meta_count < _metaurl)
                      continue;
                    else if (meta_count > _metaurl)
                      break;
                  }
           ...
           *mres_ptr && mfile->checksums && !skip_mfile; mres_ptr++)
        {
          ...
          if (!url)
          ...
          else
            {
              ...
              */
              if (!output_stream && (output_stream = fopen (destname, "rb")))
                {
                  fclose (output_stream);
...
/*
  Strip the suffix from the name, e.g. 'name.tar.xz' becomes
  'name.tar'.

  The name is permanently modified.
*/
void
strip_suffix_component (char *name)
{
  char *suffix = NULL;
